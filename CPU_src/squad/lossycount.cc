...
int ts = 0;
int dp_numNewsketch = 0;

unsigned int global1_item;
unsigned int global2_item;
...
LCU_type * LCU_Init(float fPhi, double qeps)
{
	int i;
	int k = 1 + (int) 1.0/fPhi;

	std::cout << "Initialize space saving, #entries: " << k << ", qsketch eps: "<< qeps << std::endl;
...
std::map<uint32_t, uint32_t> LCU_Output(LCU_type * lcu, int thresh)
{
	...
	for (int i=0; i<lcu->k; ++i) {
		...
		//	std::cout << "HEAVY HITTER ! count of cell: " << i << " is: " << lcu->items[i].parentg->count << std::endl;
			res.insert(std::pair<uint32_t, uint32_t>(lcu->items[i].item, lcu->items[i].parentg->count));			
		}
		//std::cout << lcu->items[i].parentg->count << std::endl;
	}
	return res;
}
...
int LCU_ComputeMem(LCU_type * lcu)
{
	...
	for (int i=0; i<lcu->k; ++i) {
		//if (flag) {
		//	std::cout << i << " gk sketch memory [byte]: " << lcu->items[i].qsketch->GetEnryMapSize()*12 << std::endl;
		//	flag = 0;
		//}
...
int returnNumNewSketch() {
	return dp_numNewsketch;
}
