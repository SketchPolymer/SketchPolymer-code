...
// Better randomness than plain rand
double __uniform() {
	// A simple function that generates a U[0,1] variable.
...
void init_skips(double p){ // pick packets w.p. p
	for (int i=0; i<(1<<16); ++i){
		m_skips[i] = log(__uniform()) / log(1-p) + 1;
	}
...
void init_fastSampling(int sample_size){
	...
	W = r * exp(1.0 / (double) sample_size);
	...
	for (int i=0; i<(1<<16); ++i){
		...
		w_skips[i] = r * exp(1.0 / (double) sample_size);
		r = ((double) rand() / (RAND_MAX)) + 1;

		logrnd_skips[i] = log(r);
		rand_skips[i] = rand() % sample_size + 1;
	}
...
LCU_type * LCU_Init(float fPhi, double qeps, long long samples_num)
{
	...
	for (i=0;i<k;i++) 
	{
		...
		result->items[i].qsketch = new Random<double>(result->qsketch_eps);
		result->items[i].vect_cnt = 0;
...
{
	int mem = 0;
	//int samples_mem = lcu->samples_num * sizeof(sampleEntry);
	int samples_mem = lcu->samples_num * 2;//TODO: 2 byte is enough to present maxmimal latency (43607)

	//int flag = 1;

	for (int i=0; i<lcu->k; ++i) {
		//if (flag) {
		//	std::cout << i << " random sketch memory: " << lcu->items[i].qsketch->sketch_memory()<< std::endl;
		//	flag = 0;
		//}
		mem += lcu->items[i].qsketch->sketch_memory();
	}

	//std::cout << "entries in ss taking in account"<< lcu->k <<", samples mem[Mbyte]: " << samples_mem / (double) (pow (10.0, 6.0)) << ", sketch mem:[Mbyte] "<< mem / (double) (pow (10.0, 6.0))<<std::endl;
...
LCUITEM * LCU_GetNewCounter(LCU_type * lcu) {
	...
	delete (newi->qsketch);
    newi->qsketch = new Random<double>(lcu->qsketch_eps);
	newi->timestamp = ts;
	newi->vect_cnt = 0;
	return (newi);
}
...
int gcount = 0;
void LCU_addToSampleFAST(LCU_type * lcu, int newitem, double latency) {
	++lcu->nb_processed;
	sampleEntry se;
	se.flow = newitem;
	se.latency = latency;
	...
	if (lcu->samples_num >= lcu->nb_processed) {
		...
		int i = newitem + floor(logrnd_skips[logrnd_skip_idx]/log(1-W)) + 1;
		//std::cout << "original flowid: " << newitem <<  " logrnd_skips[logrnd_skip_idx]: " << logrnd_skips[logrnd_skip_idx] << " the new flow is: " << i << " nb_processed: " << lcu->nb_processed << " gcount: " << gcount << std::endl;
		++logrnd_skip_idx;

		if (i <= lcu->nb_processed) {
			int j = rand_skips[rand_skip_idx];
			++rand_skip_idx;
			lcu->samples[j] = se;
			W = W * w_skips[w_skip_idx];
			++w_skip_idx;
			++gcount;
		}
	}
...
void LCU_UpdateLatencywGSample(LCU_type * lcu, int newitem, double latency) {

	if (--skip_remaining == 0){
		LCU_UpdateLatency(lcu, newitem, latency);
...
void LCU_UpdateLatencywOPT(LCU_type * lcu, int newitem, double latency) {
	int h;
	LCUITEM *il;
	...
	LCU_addToSampleFAST(lcu, newitem, latency);
}


void LCU_UpdateLatencywGSamplewOPT(LCU_type * lcu, int newitem, double latency) {

	if (--skip_remaining == 0){
		LCU_UpdateLatencywOPT(lcu, newitem, latency);
		skip_remaining = m_skips[++skip_idx];
	}	
}

/********************************************/
void LCU_UpdateLatencyOpt(LCU_type * lcu, int newitem, double latency) {
	int h;
	LCUITEM *il;
	...
	if (il==NULL) // item is not monitored (not in hashtable) 
	{
		...
		if (il->vect_cnt < item_thresh){
			++il->vect_cnt;
			il->b4_sketch_vec.push_back(latency);
		} else if (il->vect_cnt == item_thresh) {
			for(auto e:il->b4_sketch_vec) {
				il->qsketch->feed(e);
			}
		} else {
			il->qsketch->feed(latency);
...
double LCU_QuantileQuerywGsamples(LCU_type * lcu, int newitem, double rank) {
	...
	/*std::cout << "printing relevant latencies samples for: " << newitem << std::endl;
	for (auto l : sample_buff) {
		std::cout << l << std::endl;
	}*/ // For Debug

	il->qsketch->finalize_sample_w_sketch(&sample_buff);
	double val = il->qsketch->query_for_value_wSample(rank);
  	return val;
