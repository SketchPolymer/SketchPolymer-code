...
struct lcu_item 
{
  ...
  int hash;
  LCUWT delta;
  Random<double>* qsketch;//Added
  unsigned long int timestamp;//Added
  std::vector<double> b4_sketch_vec;//Optimization
  int vect_cnt;//Optimizarion
  LCUGROUP *parentg;
...
// Samples per flow
  std::unordered_map<unsigned long long, std::vector<sampleEntry>> samples_map;// flow id , vector of samples

  unsigned long long samples_num;
  unsigned long long nb_processed;
} LCU_type;

extern LCU_type * LCU_Init(float fPhi, double eps, long long samples_num);
extern void LCU_Destroy(LCU_type *);
extern void LCU_Update(LCU_type *, int);
extern void LCU_UpdateLatency(LCU_type *, int, double);
extern void LCU_UpdateLatencywOPT(LCU_type *, int, double);
extern void LCU_UpdateLatencywGSample(LCU_type * lcu, int newitem, double latency);
extern void LCU_UpdateLatencywGSamplewOPT(LCU_type * lcu, int newitem, double latency);
extern void LCU_UpdateLatencyOpt(LCU_type *, int, double);
extern double LCU_QuantileQuery(LCU_type * lcu, int newitem, double rank);
extern double LCU_QuantileQuerywGsamples(LCU_type * lcu, int newitem, double rank);
extern int LCU_ComputeMem(LCU_type * lcu);
