    ...
    void Run(uint32_t memory) 
    {
        ...
        for (int i = 0; i < running_length; ++i) 
        {
            ...
            if (id_map[dataset[i].id] > 1) 
            {
                ...
                //baseline->insert(dataset[i].id, last_time[dataset[i].id], i);
            }
            #endif
        }
        ...
        for (auto i : id_map) 
        {
            ...
            uint64_t predict = baseline->query(i.first, query_w);
    ...
    void Run(uint32_t memory) 
    {
        ...
        for (int i = 0; i < running_length; ++i) 
        {
            ...
            if (id_map[dataset[i].id] > 1 && dataset[i].timestamp > last_time[dataset[i].id]) 
            {
                ...
                //baseline->insert(dataset[i].id, last_time[dataset[i].id], dataset[i].timestamp);

                begin=clock();
                baseline->insert(dataset[i].id, dataset[i].timestamp - last_time[dataset[i].id]);
                finish=clock();
